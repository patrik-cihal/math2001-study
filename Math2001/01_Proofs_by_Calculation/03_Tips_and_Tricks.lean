...
example {u v : ℚ} (h1 : 4 * u + v = 3) (h2 : v = 2) : u = 1 / 4 :=
  calc
    u = (4*u + v + 3 * v) / 4 - v := by ring
    _ = (3 + 3 * 2) / 4 - 2 := by rw[h1, h2]
    _ = 1/4 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 = (t ^ 2 - 4) * (t^2+3*t) + (t^2-4) + 10*t +2 := by ring
    _ = 0 * (t^2+3*t) + 0 + 10*t + 2 := by rw[ht]
    _ = 10 * t + 2 := by ring
